...
var referenceTopics = map[string]string{
	...
	"checks":      "Response expressions use response, var, and steps. Span matching and span_assertions also use span and resource. Metric checks always select series with PromQL; metric_assertions use metric, whose delta is the post-trigger query value minus the pre-trigger query value. Span and metric blocks each require exactly one of at_least, at_most, or exactly.",
