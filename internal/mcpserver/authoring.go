	...
	out := capabilitiesOutput{
		...
		Checks: checkCapability{
			...
			Notes: []string{
				...
				"Keep span identity in matching and behavior in named span_assertions.",
...
var referenceTopics = map[string]string{
	...
	"checks":      "Response expressions use response, var, and steps. Span matching and span_assertions also use span and resource. A spans block has exactly one of at_least, at_most, or exactly. Prefer at_least for participation; use exact or negative counts only when cardinality and the observation window are intentional contracts.",
...
func (s *service) scaffold(_ context.Context, _ *mcp.CallToolRequest, in scaffoldInput) (*mcp.CallToolResult, scaffoldOutput, error) {
	...
	out := scaffoldOutput{
		...
		Notes: []string{
			...
			"Add response checks for the external contract and span checks only from observed trace evidence.",
